...
class AlembicImportTask():
    ...
    @staticmethod
    def _getTransforms(scale=[1,1,1], rotate=[0,0,0]):
        """Reads the transformPrefs text file to set the transforms to what they were last time the UI was used.
            Values are listed like "1 1 1 45 90 180". First 3 values are scale, last three are rotates in [X,Y,Z] order.
            
        Args:
            scale [float, float, float]: the scale values [X,Y,Z]
            rotate [float, float, float]: the rotate values [X,Y,Z]
        """
        prefsFile = os.path.join(SCRIPT_DIR,'transformPrefs.txt')
    ...
    def _setTransforms():
        ...
        Args:
            scale (float): the scale values [X,Y,Z]
            rotate (float): the rotate values [X,Y,Z]
        """
        prefsFile = os.path.join(SCRIPT_DIR,'transformPrefs.txt')
        try:
            with open(prefsFile) as file:
                readValues = file.read()
            
            transformValues = [float(num) for num in readValues.strip().split(' ')]

        except FileNotFoundError:
            transformValues = [1,1,1,0,0,0]
        print(transformValues)
    ...
    @classmethod
    def runImportsFromBlueprint(cls, files, import_folder, import_type, scale, rotate):
        """Convenience function to set queue and import files from a python call in a blueprint.
        Blueprint MUST carry all arguments of this method as pins in the blueprint python node.
        camelCase variables convert into snake_case in the bluepreint graph. The args for this function reflect that.

        Args:
            files : The filepaths for the desired imported files
            import_folder : The UE folder to import the files to.
            import_type : Defaults to ue.AlembicImportType.SKELETAL, future versions will have geometry cache and static mesh.
            scale : The scale of the imported objects.
            rotate : The rotation of the imported objects.
        ...
        Returns:
            importer : The AlembicImportTask object.
        """    
        import_type = ue.AlembicImportType.SKELETAL if import_type == 0 else ue.AlembicImportType.GEOMETRY_CACHE 
        
        importer = cls()
        
        importer.setTaskQueue(
            files,
            import_folder,
            import_type,
            scale,
        ...
        print('END IMPORT TASK:',' Completed import of ', len(files), 'files via Python.', 'Check folder located at ', import_folder, 'for assets.', end= '\n')
